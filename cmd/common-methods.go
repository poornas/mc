...
func getSourceStreamFromURL(urlStr, sseKey string) (reader io.Reader, err *probe.Error) {
...
func uploadSourceToTargetURL(ctx context.Context, urls URLs, progress io.Reader) URLs {
	...
	if sourceAlias == targetAlias {
		...
		reader, metadata, err := getSourceStream(sourceAlias, sourceURL.String(), urls.SrcSSEKey, true)
		if err != nil {
			return urls.WithError(err.Trace(sourceURL.String()))
		}
